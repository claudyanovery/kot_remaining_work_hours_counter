...
let remaining_working_hours_hh = document.getElementById(
  "remaining_working_hours_hh"
);
let remaining_working_hours_mm = document.getElementById(
  "remaining_working_hours_mm"
);
  ...
  for (const record in records) {
    ...
    for (var i = 0; i < records[record].length; i++) {
      ...
      if (records["rest_start"][i] && records["rest_end"][i]) {
        rest_time[i] = timeDiff(
          records["rest_start"][i].split(":"),
          records["rest_end"][i].split(":")
        );
...
chrome.storage.sync.get(null, (data) => {
  ...
  var total_worked_hours =
    final_records["total_work_time"] - final_records["total_rest_time"];
  ...
  if (remaining_working_hours <= 0) {
    ...
    remaining_working_hours_hh.innerHTML = msConverter(remaining_working_hours)[
      "hh"
    ];
    remaining_working_hours_mm.innerHTML = msConverter(remaining_working_hours)[
      "mm"
    ];
    ot_time_hh.innerHTML = "0";
    ot_time_mm.innerHTML = "0";
  ...
  total_rest_time_hh.innerHTML = msConverter(final_records["total_rest_time"])[
    "hh"
  ];
  total_rest_time_mm.innerHTML = msConverter(final_records["total_rest_time"])[
    "mm"
  ];
  ...
  if (final_records["lunch_time"] <= 0) {
    ...
    if (final_records["lunch_time"] === rest_time) {
      ...
      lunch_mins_left.innerHTML = msConverter(final_records["lunch_time"])[
        "mm"
      ];
    }
